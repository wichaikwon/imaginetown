...
export default function Home() {
  ...
  const [{ data, loading, error }, refetch] = useAxios('http://localhost:3000/api')
                ...
                <Stack width={250} key={idx.id}>
                  <Stack>
                    <img
                      ...
                      onClick={() => router.push(`/${idx.id}`)}
                    />
                  </Stack>
                  <Stack marginTop={1} marginBottom={1}>
                    <MovieTitle>{dayjs(idx.date).format('DD MMM YYYY')}</MovieTitle>
                  </Stack>
                  <Stack>
                    <Name>{idx[router.locale]}</Name>
                  </Stack>
                  <Stack></Stack>
                  <Stack marginTop={1}>
